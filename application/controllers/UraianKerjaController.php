...
class UraianKerjaController extends CI_Controller
{
    ...
    public function store()
    {
        ...
        for ( $i = 0; $i < (int) $this->input->post('jumlah_list_uraian_kerja'); $i++ )
        {
            ...
            $this->form_validation->set_rules("satuan[{$indexMulai}]", "Satuan ke: {$indexMulai}", 'required|in_list[unit,m,Ls]');
        ...
        $data['proyeks'] = $this->db->get($this->Proyek->table)->result();
        if ( !$this->form_validation->run() ) return $this->Helper->view('uraian_kerja/create', $data);
    ...
    public function update($uraian_kerja)
    {
        ...
        if ( !$this->form_validation->run() ) return $this->Helper->view('uraian_kerja/edit', $data);
    ...
    public function deleteall()
    {
        ...
        if (! count($deleteAllInput['tdSelectedUraianKerja']) && count($deleteAllInput['tdSelectedUraianKerjaDetails']) ) { $this->session->set_flashdata('error', "Tidak ada uraian kerja yang dihapus!"); redirect('uraian_kerja'); }
    ...
    public function tampilkanBerdasarkanProyek($proyek)
    {
