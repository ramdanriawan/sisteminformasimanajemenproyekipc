...
class TagihanController extends CI_Controller
{
    ...
    {
        $this->Helper->dd($this->input->post('id_proyek').'ll');
        $data['datas'] = $this->db->where(['proyek_id' => $this->input->post('id_proyek_id')])->get($this->Tagihan->table)->result();
        $data['proyeks'] = $this->db->get($this->Proyek->table, 100)->result();
        $data['proyek_id'] = $this->input->post('proyek_id');
